...
export default function AdminPage() {
  ...
  const handleTeamSubmit = async (e: React.FormEvent) => {
    ...
    if (!teamForm.name.trim() || !teamForm.league.trim()) {
      toast({
        title: "Error",
        description: "Please fill in both team name and league",
        variant: "destructive",
      })
      return
    }
    ...
    try {
      const newTeam = await api.createTeam(teamForm.name.trim(), teamForm.league.trim())
        ...
        {connectionStatus === "disconnected" && (
          <Alert className="mb-6">
            ...
            <AlertDescription className="flex items-center gap-2">
              ...
              Backend server not connected. Form submissions will use mock responses.
            </AlertDescription>
          </Alert>
        )}
            ...
            <CardContent>
              <form onSubmit={handleMatchSubmit} className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  {["home_team_id", "away_team_id"].map((key, idx) => (
                    <div key={key}>
                      <Label>{idx === 0 ? "Home Team" : "Away Team"}</Label>
                      <Select
                        value={matchForm[key as keyof typeof matchForm]}
                        onValueChange={(value) => setMatchForm((prev) => ({ ...prev, [key]: value }))}
                      >
                        <SelectTrigger>
                          <SelectValue placeholder={`Select ${idx === 0 ? "home" : "away"} team`} />
                        </SelectTrigger>
                        <SelectContent>
                          {teams.map((team) => (
                ...
                <div className="grid grid-cols-2 gap-4">
                  {["home_score", "away_score"].map((key, idx) => (
                    <div key={key}>
                      <Label>{idx === 0 ? "Home Score" : "Away Score"}</Label>
                      <Input
                        type="number"
                        min="0"
                        value={matchForm[key as keyof typeof matchForm]}
                        onChange={(e) => setMatchForm((prev) => ({ ...prev, [key]: e.target.value }))}
                        required
                      />
                    </div>
                  ))}
