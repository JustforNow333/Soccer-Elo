import requests
import pandas as pd
from io import StringIO
from datetime import datetime, date
from db import db, Team, Match, EloRating
from elo_utils import update_elo, get_match_result
import numpy as np
import re

# League code to league name mapping
LEAGUE_MAPPING = {
    # Club World Cup
    "CWC": "Club World Cup",
    # England
    "E0": "Premier League",
    "E1": "Championship", 
    "E2": "League One",
    "E3": "League Two",
    "EC": "Conference",
    
    # Scotland  
    "SC0": "Scottish Premier League",
    "SC1": "Scottish Division 1",
    "SC2": "Scottish Division 2", 
    "SC3": "Scottish Division 3",
    
    # Germany
    "D1": "Bundesliga",
    "D2": "2. Bundesliga",
    
    # Italy
    "I1": "Serie A",
    "I2": "Serie B",
    
    # Spain
    "SP1": "La Liga",
    "SP2": "La Liga 2",
    
    # France
    "F1": "Ligue 1",
    "F2": "Ligue 2",
    
    # Netherlands
    "N1": "Eredivisie",
    
    # Belgium
    "B1": "Belgian Pro League",
    
    # Portugal
    "P1": "Primeira Liga",
    
    # Turkey
    "T1": "Süper Lig",
    
    # Greece
    "G1": "Super League Greece",
    
    # Austria
    "A1": "Austrian Bundesliga",
    
    # Switzerland
    "CH1": "Swiss Super League",
    
    # Russia
    "RU1": "Russian Premier League",
}

def extract_league_from_url(url):
    """Extract league code from football-data.co.uk URL and return league name."""
    # Example URL: https://www.football-data.co.uk/mmz4281/0001/E0.csv
    match = re.search(r'/([A-Z]+[0-9]*|[A-Z]+)\.csv$', url)
    if match:
        league_code = match.group(1)
        return LEAGUE_MAPPING.get(league_code, f"Unknown ({league_code})")
    return "Unknown"


def fetch_csv(url):
    print(f"Fetching: {url}")
    try:
        res = requests.get(url)
        if res.ok:
            # Add error handling for malformed CSV files
            try:
                return pd.read_csv(StringIO(res.text), on_bad_lines='skip')
            except pd.errors.ParserError as e:
                print(f"Parser error for {url}: {e}")
                return None
        else:
            print(f"Failed to fetch: {url}")
            return None
    except Exception as e:
        print(f"Error fetching {url}: {e}")
        return None


import unicodedata

def normalize_team_name(name):
    if not name:
        return ""
    # Remove accents, lowercase, strip whitespace
    name = unicodedata.normalize('NFKD', name)
    name = "".join([c for c in name if not unicodedata.combining(c)])
    return name.strip().lower()

def get_or_create_team(name, league="Unknown"):
    if not name or str(name).lower() == 'nan':
        print(f"Skipping team with invalid name: {name}")
        return None
    norm_name = normalize_team_name(name)
    team = Team.query.filter_by(name=norm_name).first()
    if not team:
        team = Team(name=norm_name, league=league)
        db.session.add(team)
        db.session.commit()
        print(f"Created team: {norm_name} in {league}")
    else:
        # Update league if it's currently "Unknown" or different
        if team.league == "Unknown" or team.league != league:
            old_league = team.league
            team.league = league
            db.session.commit()
            print(f"Updated {norm_name}: {old_league} -> {league}")
    return team


def is_valid_team_name(val):
    try:
        return val is not None and str(val).strip() and str(
            val).lower() != "nan"
    except Exception:
        return False


def import_matches_from_csv(url):
    from app import app
    with app.app_context():
        df = fetch_csv(url)
        if df is None:
            return

        required_cols = {"Date", "HomeTeam", "AwayTeam", "FTHG", "FTAG"}
        if not required_cols.issubset(df.columns):
            print(f"Missing required columns in {url}")
            return

        elo_cache = {}

        def get_latest_elo(team_id):
            if team_id not in elo_cache:
                latest_elo = EloRating.query.filter_by(team_id=team_id).order_by(
                    EloRating.date.desc()).first()
                elo_cache[team_id] = latest_elo.rating if latest_elo else 1000
            return elo_cache[team_id]

        matches_to_add = []
        elos_to_add = []

        for _, row in df.iterrows():
            try:
                home_team = row["HomeTeam"]
                away_team = row["AwayTeam"]

                home_team_str = str(home_team).strip() if is_valid_team_name(home_team) else ""
                away_team_str = str(away_team).strip() if is_valid_team_name(away_team) else ""

                # Normalize team names for cross-competition consistency
                home_team_str = normalize_team_name(home_team_str)
                away_team_str = normalize_team_name(away_team_str)

                if not home_team_str or not away_team_str:
                    print(f"Skipping row with missing or invalid team name: {row}")
                    continue

                date_val = row["Date"]
                parsed = pd.to_datetime(date_val, dayfirst=True, errors="coerce")
                if pd.isnull(parsed):
                    print(f"Skipping row with invalid date: {row}")
                    continue
                match_date = parsed.date()

                league = extract_league_from_url(url)
                home = get_or_create_team(home_team_str, league)
                away = get_or_create_team(away_team_str, league)
                if home is None or away is None:
                    continue

                # ✅ Safe check for duplicate match on same date between same teams
                existing_match = Match.query.filter_by(
                    date=match_date,
                    home_team_id=home.id,
                    away_team_id=away.id
                ).first()
                if existing_match:
                    continue

                match = Match(date=match_date,
                              home_team_id=home.id,
                              away_team_id=away.id,
                              home_score=int(row["FTHG"]),
                              away_score=int(row["FTAG"]))
                matches_to_add.append(match)

                # Elo updates
                from elo_utils import get_match_result, update_elo
                home_score, away_score = get_match_result(match.home_score, match.away_score)

                home_rating_val = get_latest_elo(home.id)
                away_rating_val = get_latest_elo(away.id)

                new_home_rating = update_elo(home_rating_val, away_rating_val, home_score)
                new_away_rating = update_elo(away_rating_val, home_rating_val, away_score)

                elo_cache[home.id] = new_home_rating
                elo_cache[away.id] = new_away_rating

                elos_to_add.append(EloRating(team_id=home.id, date=match_date, rating=new_home_rating))
                elos_to_add.append(EloRating(team_id=away.id, date=match_date, rating=new_away_rating))

            except Exception as e:
                print(f"Error processing row: {e}")

        if matches_to_add:
            db.session.add_all(matches_to_add)
            db.session.add_all(elos_to_add)
            db.session.commit()
            print(f"Added {len(matches_to_add)} new matches and updated ELOs.")
        else:
            print("No new matches found.")


def generate_football_data_urls(start_season=1993, end_season=2025, league_codes=None, include_club_world_cup=True):
    """
    Generate all football-data.co.uk CSV URLs for given seasons and leagues.
    """
    if league_codes is None:
        # Most common European leagues (add more as needed)
        league_codes = [
            "E0",
            "E1",
            "E2",
            "E3",
            "EC",  # England
            "SC0",
            "SC1",
            "SC2",
            "SC3",  # Scotland
            "D1",
            "D2",  # Germany
            "I1",
            "I2",  # Italy
            "SP1",
            "SP2",  # Spain
            "F1",
            "F2",  # France
            "N1",  # Netherlands
            "B1",  # Belgium
            "P1",  # Portugal
            "T1",  # Turkey
            "G1",  # Greece
            "A1",  # Austria
            "CH1",  # Switzerland
            "RU1",  # Russia
        ]
    urls = []
    for year in range(start_season, end_season):
        # e.g., 2000-01 season is 0001, 2022-23 is 2223
        season = f"{str(year)[-2:]}{str(year+1)[-2:]}"
        for code in league_codes:
            url = f"https://www.football-data.co.uk/mmz4281/{season}/{code}.csv"
            urls.append(url)
    # Add Club World Cup CSV URL(s) if requested
    if include_club_world_cup:
        # Example: Replace with actual Club World Cup CSV source if available
        # Here we use a placeholder URL; user should update as needed
        for year in range(start_season, end_season):
            # Example: https://example.com/club_world_cup/YYYY.csv
            cwc_url = f"https://www.football-data.co.uk/clubworldcup/{year}.csv"
            urls.append(cwc_url)
    return urls


if __name__ == "__main__":
    # Generate all URLs for all leagues/seasons
    # Note: This standalone script needs to import app here to avoid circular imports
    from app import app
    
    # Ensure database tables are created before importing
    with app.app_context():
        db.create_all()

    urls = generate_football_data_urls(start_season=1993, end_season=2025)
    for url in urls:
        import_matches_from_csv(url)
    print("All matches and Elo ratings imported successfully!")
