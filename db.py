...
class Team(db.Model):
    ...
    def serialize(self):
        # Get the latest ELO rating for this team
        latest_elo = EloRating.query.filter_by(team_id=self.id).order_by(EloRating.date.desc()).first()
        
        return {
            "id": self.id,
            "name": self.name,
            "league": self.league,
            "elo": latest_elo.rating if latest_elo else None,
            # Remove the massive elo_ratings array that's causing the 14MB response
            # "elo_ratings": [e.simple_serialize() for e in list(self.elo_ratings)]
        }
