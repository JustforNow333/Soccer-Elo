#!/usr/bin/env python3
"""
Complete Setup Script - Does initial import then starts scheduler
Combines run_complete_import.py + scheduler for full automation
"""
...
def run_complete_setup():
    """Run complete import followed by scheduler"""
    print("🚀 Starting COMPLETE setup: Import + Scheduler...")
    print("="*70)
    
    # Phase 1: Complete Import
    print("\n📦 PHASE 1: Complete Historical Import")
    print("-" * 50)
    
    success = run_complete_import()
    if not success:
        print("❌ Import failed - stopping setup")
        return False
    
    print("\n✅ Import completed successfully!")
    print("💤 Waiting 10 seconds before starting scheduler...")
    time.sleep(10)
    
    # Phase 2: Start Scheduler  
    print("\n📅 PHASE 2: Starting Daily Scheduler")
    print("-" * 50)
...
def run_complete_import():
    """Run the complete import process (copied from run_complete_import.py)"""
...
def start_scheduler():
    """Start the daily scheduler for ongoing updates"""
    try:
        from apscheduler.schedulers.blocking import BlockingScheduler
        
        # Import the functions we need
        print("🔧 Initializing scheduler functions...")
        
        api_key = os.environ.get('API_FOOTBALL_KEY')
        if not api_key:
            print("❌ API_FOOTBALL_KEY not found for scheduler")
            return False
            
        def update_fixtures():
            """Update fixtures for the next week for top 250 teams"""
            print("📅 Running daily fixture update...")
            try:
                from app import app
                from api_import import APIFootballImporter
                ...
                with app.app_context():
                    ...
                    print("✅ Daily fixture update complete!")
            except Exception as e:
                print(f"❌ Daily fixture update failed: {e}")

        def update_recent_matches():
            """Update recent matches for top 250 teams"""
            print("⚡ Running 5-minute match update...")
            try:
                from app import app
                from api_import import APIFootballImporter
                ...
                with app.app_context():
                    ...
                    print("✅ Recent match update complete!")
            except Exception as e:
        ...
        print("❌ APScheduler not available - cannot start scheduler")
...
if __name__ == "__main__":
    print("🎯 Complete Setup: Import + Scheduler")
    print("This will:")
    print("  1️⃣  Map all 250 teams")
    print("  2️⃣  Import historical data from 2000")
    print("  3️⃣  Calculate ELO ratings")
    print("  4️⃣  Start daily scheduler")
    print()
